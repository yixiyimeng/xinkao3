<template>
	<div v-if="showVote" class="bg">
			...
			<div>
				<a-form :form="form">
					<a-form-item label="评分主题" :label-col="{ span: 4}" :wrapper-col="{ span: 20 }">
						<a-input class="lginput" v-decorator="['title', { rules: [{ required: true, message: '请输入评分主题!' }] }]"
					...
					<a-form-item label="评分对象" :label-col="{ span: 4 }" :wrapper-col="{ span:20 }">
						<div v-for="item in objlist" :key="item" class="objitem">
							<a-input v-decorator="['obj['+item+']', { rules: [{ required: true, message: '请输入评分对象!' }] }]" placeholder="评分对象"
		...
		<div class="voteChart flex flex-align-center" v-if="isAnswering">
			<div class="voteInfo">
				<div class="flex"><label>评分主题:</label><span class="flex-1">{{voteInfo.title}}</span></div>
