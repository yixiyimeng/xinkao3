	...
	<div v-if="showVote" class="bg">
				...
				<a-form :form="form">
					<a-form-item label="投票主题" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
						<a-input
							...
							v-decorator="[
								'titleName',
								{
									rules: [{ required: true, message: '请输入投票主题!' }, { max: 20, whitespace: true, message: '主题字数不能超过20!' }],
					...
					<a-form-item label="投票描述" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
						<a-textarea
							...
							placeholder="请输入投票描述"
							:rows="4"
							v-decorator="['describe', { rules: [{ required: true, whitespace: true, message: '请输入投票描述!' }, { max: 100, message: '描述字数不能超过100!' }] }]"
						/>
					</a-form-item>
						...
						<div class="objitem">
							<a-input
								v-decorator="[
									'objs[' + item + ']',
									{
										rules: [{ required: true, whitespace: true, message: '请输入投票对象!' }, { max: 5, message: '字数最多5个!' }],
										getValueFromEvent: event => {
											return event.target.value.replace(/\s+/g, '');
										}
									}
		...
		<div class="voteChart" v-if="viewState != 0" :class="{ setbox: isChart }">
			<div class="flex flex-align-center">
				...
				<div class="flex-1" v-if="isChart"><v-chart :options="polar" autoresize class="chartbox" @click="showName"></v-chart></div>
			</div>
		</div>
		<div class="btnbar">
