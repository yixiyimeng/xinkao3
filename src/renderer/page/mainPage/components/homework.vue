				...
				<div class="modbox flex flex-v" ref="modbox" style="margin: 0 20px; height: 100%;top:0;position: relative;">
					...
					<div class="tablelist mt20 flex-1" ref="setbox">
						...
						>
							...
							<a-tag :color="state == 0 ? '#00a095' : state == 1 ? '#ff8d1a' : '#d43030'" slot="testPaperStatus" slot-scope="state">
								{{ state == 0 ? '未发送' : state == 1 ? '已发送' : '已收取' }}
							</a-tag>
							...
							<span slot="action" slot-scope="text, record, index">
								...
								<!-- <a href="javascript:;" v-if="record.testPaperStatus == 2" @click="showSchoolreport(record)">成绩单</a> -->
							</span>
						</a-table>
					</div>
				</div>
