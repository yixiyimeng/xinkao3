...
export default {
	...
	mounted() {
		...
		window.onresize = function() {
			// that.scrolly = document.body.offsetHeight * .6 - 180;
			that.tabheight = that.$refs.modbox.offsetHeight -( this.theme == 'theme4' ? 150 : 65);
		};
		this.$nextTick(() => {
			if (this.$refs.modbox) {
				this.tabheight = this.$refs.modbox.offsetHeight - ( this.theme == 'theme4' ? 150 : 65);
				// this.scrolly = this.tabheight - 120;
