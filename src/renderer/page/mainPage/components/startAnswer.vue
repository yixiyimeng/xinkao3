...
export default {
	...
	methods: {
		...
		startChoice() {
			...
			$me.$postAction(api.startSingleAnswer, param)
				.then(da => {
					if (da && da.ret == 'success') {
						/* 开始答题 */
						/* 清空倒计时 */
		...
		returnback() {
			if (this.viewState == 2) {
				if (this.$refs.startClassTesting.isShow) {
					this.$refs.startClassTesting.returnback();
				} else {
		...
		saveImgFullScreen() {
			/* 全屏截图 */
			const $me = this;
			$me.$postAction(api.saveQuestionImgAndSend)
				.then(da => {
					if (da && da.ret == 'success') {
					} else {
